...
class RecordService:
    ...
    async def create_record(self, user_id: str, word_count: int, study_time: int, timestamp: int | None) -> None:
        ...
        record_id = self.__generate_record_id(user_id, word_count, study_time, now)
        ...
        await self.__record_repo.create_record(user.user_id, record_id, word_count, study_time, now)
        return
